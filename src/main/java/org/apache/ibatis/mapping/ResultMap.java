...
public class ResultMap {
  ...
  private String id; //resultMap的id
  private Class<?> type; //resultMap的type
  private List<ResultMapping> resultMappings; //除discriminator以外的映射关系
  private List<ResultMapping> idResultMappings; //记录<id>或者<constructor>中的<idArg>的映射关系
  private List<ResultMapping> constructorResultMappings; //记录<constructor>中的<result>的映射关系
  private List<ResultMapping> propertyResultMappings; //记录非<constructor>标志的映射关系
  private Set<String> mappedColumns; //记录所有有映射关系的columns字段，这东西的作用是如果开启自动映射，没配映射关系的字段采用自动映射
  ...
  private boolean hasNestedQueries; //是否有嵌套查询映射
